...
class ShedFixers(VisitorBasedCodemodCommand):
    ...
    def use_set_or_dict_literals(self, _, updated_node):
        """Fix flake8-comprehensions C405-406.

        Unnecessary <list/tuple> literal - rewrite as a <set/dict> literal.
        """
        # Note: only need to handle dict literals; pyupgrade does the sets
        return updated_node

    def use_dict_or_list_or_tuple_literals(self, _, updated_node):
        """Fix flake8-comprehensions C409-410.

        Unnecessary <list/tuple> passed to <list/tuple>() -
        (remove the outer call to <list/tuple>``()/rewrite as a ``<list/tuple> literal).

        TODO: also handle sets and dicts (and propose checking upstream?)
        """
        # TODO: this might be implemented as part of the above?
        return updated_node

    def replace_unnecessary_dict_call_around_comprehension(self, _, updated_node):
        """Fix a problem similar to flake8-comprehensions C411.

        List and set comps are handled by replace_unnecessary_list_around_sorted().
        """
        return updated_node
    ...
    def replace_unnecessary_dict_call_around_comprehension_or_literal(
        self, _, updated_node
    ):
        # TODO: generalise this logic to include calls to set and dict around those
        #       comprehensions, and to literals as well as comprehensions.
        return updated_node
    ...
    def replace_unnecessary_reversed_around_sorted(self, _, updated_node):
        ...
        assert call
        # TODO: invert reverse= argument, adding if needed; removing if now False
        return updated_node
